...
chrome.tabs.onMoved.addListener(
  ...
  (id, {windowId, fromIndex, toIndex}) => {
    ...
    get(id).then(tab =>
      invoke('moved', listener => listener(tab, windowId, fromIndex, toIndex))
    );
  }
);
...
chrome.tabs.onAttached.addListener(
  ...
  (id, {newWindowId, newPosition}) => {
    ...
    get(id).then(tab =>
      invoke('attached', listener => listener(tab, newWindowId, newPosition))
    );
  }
);
...
chrome.tabs.onDetached.addListener(
  ...
  (id, {oldWindowId, oldPosition}) => {
    ...
    get(id).then(tab =>
      invoke('detached', listener => listener(tab, oldWindowId, oldPosition))
    );
  }
);
...
chrome.tabs.onActivated.addListener(
  ...
  ({tabId, windowId}) => {
    ...
    get(tabId).then(tab =>
      invoke('activated', listener => listener(tab, windowId))
    );
  }
);
...
chrome.tabs.onHighlighted.addListener(
  ...
  ({tabIds, windowId}) => {
    ...
    Promise.all(tabIds.map(get)).then(tabs =>
      invoke('highlighted', listener => listener(tabs, windowId))
    );
  }
);
...
chrome.tabs.onZoomChange.addListener(
  ...
  ({tabId, oldZoomFactor, newZoomFactor, zoomSettings}) => {
    ...
    get(tabId).then(tab =>
      invoke('zoomchange', listener => listener(
        tab,
        oldZoomFactor,
        newZoomFactor,
        zoomSettings
